...
let handle_connection ic oc _ =
  ...
  | Some req -> (
      try%lwt
        ...
        (match resp with
        | Some resp -> Lwt_io.write_line oc resp
        | None -> Lwt.return ())
      with e ->
...
let create_socket () =
  ...
  let%lwt () = bind sock @@ ADDR_INET(address, 2239) in
  listen sock backlog;
  Lwt.return sock
